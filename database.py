...
async def next_voucher_id(prefix: str, date_str: str) -> str:
    ...
    Counts existing voucher_ids that start with 'VR-{prefix}-{date_str}'
    to determine the next sequence number. Uses a single DB query so it is
    safe under async concurrency (SQLite serialises writes).
    """
...
async def list_voucher_records(
    ...
    try:
        ...
        if keyword:
            like = f"%{keyword}%"
            conditions.append(
                "(vr.header_text LIKE ? OR vr.reference LIKE ?)"
            )
...
async def approve_voucher(voucher_id: str, action_by: str, comment: str = "") -> bool:
    """Approve a pending_approval voucher → posted."""
    db = await get_db()
    try:
        now = datetime.now().isoformat()
        cursor = await db.execute(
...
async def reject_voucher(voucher_id: str, action_by: str, comment: str) -> bool:
    """Reject a pending_approval voucher → draft."""
    db = await get_db()
    try:
        now = datetime.now().isoformat()
        cursor = await db.execute(
...
async def get_approval_record(voucher_id: str) -> dict | None:
    ...
    try:
        cursor = await db.execute(
            """SELECT ar.*, u1.display_name AS requester_name, u2.display_name AS approver_name
...
async def list_pending_approvals(approver_id: str) -> list[dict]:
    """List all vouchers pending approval by a specific approver."""
    db = await get_db()
    try:
        cursor = await db.execute(
            """SELECT vr.voucher_id, vr.company_code, vr.document_type, vr.document_date,
                      vr.header_text, vr.reference, vr.confidence, vr.created_at,
                      ar.requested_by, ar.created_at AS submitted_at,
